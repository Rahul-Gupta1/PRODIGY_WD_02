...
const updateTime = () => {
  ...
  $(".pause-stopwatch").click(function () {
    if (stopwatchRunning) {
        pausedTime = stopwatchHours * 3600 + stopwatchMinutes * 60 + stopwatchSeconds;
        stopStopwatch();
        $(this).text("Resume");
    } else {
        stopwatchHours = Math.floor(pausedTime / 3600);
        stopwatchMinutes = Math.floor((pausedTime % 3600) / 60);
        stopwatchSeconds = Math.floor(pausedTime % 60);
        startStopwatch();
        $(this).text("Pause");
    }
  });
